...
class GeneralParserV2
{
     ...
     * @return mixed
     */
    public function compareNameWithNextLines($k,$htmlFile): int
    {
        try {
                    ...
                    for ($j = 1; $j <= $product_row_length ; $j++) {
                       ...
                       // echo '==='.$j.'===';
                        ...
                        if ( isset($parser_parameters['products_columns']['product_no']) && $parser_parameters['products_columns']['product_no'] == $j) {
                            ...
                            $name = str_replace($result, '', $product_no);
                            if($result == '')
                        ...
                        if (isset($parser_parameters['products_columns']['name']) && $parser_parameters['products_columns']['name'] == $j) {
                           // echo 'name';
                            ...
                            $productList[$index]['name'] = $name.$productList[$index]['name'];
                            echo $productList[$index]['name'];
...
//                        }
                        ...
                        if (isset($parser_parameters['products_columns']['price']) && $parser_parameters['products_columns']['price'] == $j) {
                            ...
                            if(strstr($productList[$index]['price'], ','))
                                $productList[$index]['price'] = floatval(str_replace(['.', ','],['','.'],$productList[$index]['price']));
                            else
                                $productList[$index]['price'] = floatval($productList[$index]['price']);
                        ...
                        if (isset($parser_parameters['products_columns']['product_total']) && $parser_parameters['products_columns']['product_total'] == $j) {
                            ...
                            while(empty($productList[$index]['total']) || $productList[$index]['total'] == '0.0')
                            {
                                $i++;
                                $k = $i + $j;
                                $productList[$index]['total'] = floatval(str_replace(['Â ',"\r\n","\r","\n",' '], '', strip_tags($htmlFile[$k-1])));
                            }

                            if(strstr($productList[$index]['total'], ','))
                                $productList[$index]['total'] = floatval(str_replace(['.', ','],['','.'],$productList[$index]['total']));
                            else
                                $productList[$index]['total'] = floatval($productList[$index]['total']);
